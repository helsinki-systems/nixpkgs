...
let
  ...
  config = let {
    ...
    pathExists = name: builtins ? pathExists && builtins.pathExists (toPath name);
    ...
    body =
      if configFile != "" && pathExists configFile then
        import (toPath configFile)
      else if homeDir != "" && pathExists configFile2 then
        import (toPath configFile2)
      else
        { };
  };
  ...
  pkgsFun = __overrides: rec {
    ...
    addAttrsToDerivation =
      extraAttrs: stdenv: stdenv // { mkDerivation = args: stdenv.mkDerivation (args // extraAttrs); };
    ...
    useFromStdenv =
      it: alternative: if (builtins.hasAttr it stdenv) then (builtins.getAttr it stdenv) else alternative;
    ...
    checker =
      ...
      (
        if flag then
          let
            result = ((import ../build-support/checker) opts config);
          in
          (if (result == "") then x else abort ("Unknown option specified: " + result))
        else
          x
      );
    ...
    stdenv_32bit = if system == "x86_64-linux" then overrideGCC stdenv gcc43multi else stdenv;
    ...
    symlinkJoin =
      ...
      runCommand name {
        inherit paths;
      } "mkdir -p $out; for i in $paths; do ${xorg.lndir}/bin/lndir $i $out; done";
    ...
    aria =
      builderDefsPackage (import ../tools/networking/aria)
        {
        };
    ...
    coreutils = useFromStdenv "coreutils" (
      (if stdenv ? isDietLibC then import ../tools/misc/coreutils-5 else import ../tools/misc/coreutils) {
    ...
    fcron = import ../tools/system/fcron {
      # see also cron
      inherit fetchurl stdenv perl;
    };
    ...
    findutilsWrapper = lowPrio (
      appendToName "wrapper" (
        import ../tools/misc/findutils-wrapper {
    ...
    glxinfo =
      assert mesaSupported;
      import ../tools/graphics/glxinfo {
        inherit
          fetchurl
          stdenv
          x11
    ...
    gnupatch = useFromStdenv "patch" (
      import ../tools/text/gnupatch {
        inherit fetchurl stdenv;
      }
    );
    ...
    */
    ...
    ppp =
      builderDefsPackage (import ../tools/networking/ppp)
        {
        };
    ...
    */
    ...
    bashInteractive = appendToName "interactive" (
      import ../shells/bash/new.nix {
        inherit
          fetchurl
          stdenv
          ncurses
    ...
    aspectj = import ../development/compilers/aspectj {
    ...
    dylan = import ../development/compilers/gwydion-dylan {
      inherit
        ...
        readline
        ;
      dylan = import ../development/compilers/gwydion-dylan/binary.nix {
        inherit fetchurl stdenv;
      };
    };
    ...
    gcc295 = wrapGCC (
      import ../development/compilers/gcc-2.95 {
        inherit fetchurl stdenv noSysDirs;
      }
    );

    gcc33 = wrapGCC (
      import ../development/compilers/gcc-3.3 {
        inherit fetchurl stdenv noSysDirs;
      }
    );

    gcc34 = wrapGCC (
      import ../development/compilers/gcc-3.4 {
        inherit fetchurl stdenv noSysDirs;
      }
    );
    ...
    gcc40 = wrapGCC (
      import ../development/compilers/gcc-4.0 {
        inherit fetchurl stdenv noSysDirs;
        texinfo = texinfo49;
    ...
    gcc41 = wrapGCC (
      import ../development/compilers/gcc-4.1 {
        inherit fetchurl stdenv noSysDirs;
        texinfo = texinfo49;
    ...
    gcc42 = useFromStdenv "gcc" (
      wrapGCC (
        import ../development/compilers/gcc-4.2 {
          inherit fetchurl stdenv noSysDirs;
          profiledCompiler = true;
        }
      )
    ...
    gcc43 = lowPrio (
      wrapGCCWith (import ../build-support/gcc-wrapper-new) glibc (
        import ../development/compilers/gcc-4.3 {
          inherit
            fetchurl
            stdenv
            texinfo
    ...
    gcc43multi = lowPrio (
      wrapGCCWith (import ../build-support/gcc-wrapper-new) glibc_multi (
        import ../development/compilers/gcc-4.3 {
    ...
    gccApple = wrapGCC (
      import ../development/compilers/gcc-apple {
        inherit fetchurl stdenv noSysDirs;
        profiledCompiler = true;
      }
    );
    ...
    gccupc40 = wrapGCCUPC (
      import ../development/compilers/gcc-upc-4.0 {
        inherit
          fetchurl
          stdenv
          bison
    ...
    ghcWrapper =
      {
        ghcPackagedLibs ? false,
        ghc,
        libraries,
        name,
        suffix ? "ghc_wrapper_${ghc.name}",
      }:
      import ../development/compilers/ghc/ghc-wrapper {
        ...
        installSourceAndTags = getConfig [
          "haskell"
          "ghcWrapper"
          "installSourceAndTags"
        ] false;
      };
    ...
    ghcCabalDerivation =
      ...
      let
        buildInputs =
          ...
          ++ [ ghcPkgUtil ]
          ++ (if args ? pass && args.pass ? buildInputs then args.pass.buildInputs else [ ]);
        configure =
          if (args ? useLocalPkgDB) then
            "nix_ghc_pkg_tool join localDb\n"
            + "\$CABAL_SETUP configure --package-db=localDb \$profiling \$cabalFlags"
          else
            "\$CABAL_SETUP configure --by-env=\$PACKAGE_DB \$profiling \$cabalFlags";
      in
      stdenv.mkDerivation (
        {
    ...
    ghcCabalExecutableFun =
      (import ../development/compilers/ghc/ghc-wrapper/ghc-cabal-executable-fun.nix)
        {
          inherit
            ghc68extraLibs
            ghcsAndLibs
    ...
    ghc68executables = recurseIntoAttrs (
      import ../misc/ghc68executables {
        inherit
          ghcCabalExecutableFun
          fetchurl
          lib
    ...
    ghc69snapshot = lowPrio (
      import ../development/compilers/ghc-6.8/head.nix {
        inherit
          fetchurl
          stdenv
          readline
    ...
    ghcboot = lowPrio (
      appendToName "boot" (
        import ../development/compilers/ghc/boot.nix {
          inherit
            fetchurl
            stdenv
            ncurses
    ...
    ghcboot610 = lowPrio (
      appendToName "boot" (
        import ../development/compilers/ghc/boot610.nix {
          inherit
            fetchurl
            stdenv
            ncurses
    ...
    */
    ...
    supportsJDK = system == "i686-linux" || system == "x86_64-linux" || system == "powerpc-linux";

    jdkdistro =
      installjdk: pluginSupport:
      assert supportsJDK;
    ...
    monotoneViz =
      builderDefsPackage (selectVersion ../applications/version-management/monotone-viz "1.0.1")
        {
          inherit
            ocaml
            lablgtk
    ...
    */
    ...
    strategoLibraries =
      import ../development/compilers/strategoxt/libraries/stratego-libraries-0.17pre.nix
        {
          inherit
            stdenv
            fetchurl
    ...
    wrapGCCUPC =
      baseGCC:
      import ../build-support/gcc-upc-wrapper {
        nativeTools = stdenv ? gcc && stdenv.gcc.nativeTools;
        nativeLibc = stdenv ? gcc && stdenv.gcc.nativeLibc;
        gcc = baseGCC;
        libc = glibc;
    ...
    phpOld = import ../development/interpreters/php {
      inherit
        ...
        apacheHttpd
        ;
      unixODBC = if stdenv.isDarwin then null else unixODBC;
    };
    ...
    */
    ...
    cmakeUnstable = lowPrio (
      import ../development/tools/build-managers/cmake/2.6.3.nix {
        inherit
          fetchurl
          stdenv
          replace
    ...
    elfutils = composedArgsAndFun (selectVersion ../development/tools/misc/elfutils "0.131") {
      inherit fetchurl stdenv;
    };
    ...
    haddockboot = lowPrio (
      appendToName "boot" (
        import ../development/tools/documentation/haddock/boot.nix {
          inherit gmp;
          cabal = cabalboot;
        }
      )
    ...
    haddock210 = lowPrio (
      import ../development/tools/documentation/haddock/haddock-2.1.0.nix {
        cabal = cabal682;
      }
    );
    ...
    hsc2hs = import ../development/tools/misc/hsc2hs {
      ...
      ghc = ghcsAndLibs.ghc68.ghc;
      libs = with (ghc68extraLibs ghcsAndLibs.ghc68 // ghcsAndLibs.ghc68.core_libs); [
        base
        directory
        process
        cabal_darcs
    ...
    */
    ...
    libtoolOld = lowPrio (
      import ../development/tools/misc/libtool/old.nix {
        inherit
          fetchurl
          stdenv
          perl
    ...
    */
    ...
    swigWithJava = lowPrio (
      appendToName "with-java" (
        import ../development/tools/misc/swig {
          inherit fetchurl stdenv jdk;
          perlSupport = false;
          pythonSupport = false;
          javaSupport = true;
    ...
    aspellDicts = recurseIntoAttrs (
      import ../development/libraries/aspell/dictionaries.nix {
        inherit
          fetchurl
          stdenv
          aspell
    ...
    aterm24 = lowPrio (
      import ../development/libraries/aterm/2.4.nix {
        inherit fetchurl stdenv;
      }
    );
    ...
    aterm27 = lowPrio (
      import ../development/libraries/aterm/2.7.nix {
        inherit fetchurl stdenv;
      }
    );
    ...
    boostVersionChoice =
      version:
      selectVersion ../development/libraries/boost version {
        inherit
          fetchurl
          stdenv
          icu
    ...
    clppcre =
      builderDefsPackage (import ../development/libraries/cl-ppcre)
        {
        };
    ...
    freeglut =
      assert mesaSupported;
      import ../development/libraries/freeglut {
        inherit
          fetchurl
          stdenv
          x11
    ...
    glibcUnstable = lowPrio (
      import ../development/libraries/glibc-2.8 {
        inherit fetchurl stdenv kernelHeaders;
    ...
    gst_all = recurseIntoAttrs (
      import ../development/libraries/gstreamer {
        inherit
          lib
          selectVersion
          stdenv
        ...
        inherit (gnome)
          gnomevfs
          /*
            <- only passed for the no longer used older versions
            it is depreceated and didn't build on amd64 due to samba dependenccy
          */
          gtkdoc
          ;
      }
    ...
    libcm =
      assert mesaSupported;
      import ../development/libraries/libcm {
        inherit
          fetchurl
          stdenv
          pkgconfig
    ...
    libdbiDriversBase =
      composedArgsAndFun (selectVersion ../development/libraries/libdbi-drivers "0.8.2-1")
        {
          inherit
            stdenv
            fetchurl
    ...
    libjpegStatic = lowPrio (
      appendToName "static" (
        import ../development/libraries/libjpeg-static {
          inherit fetchurl stdenv libtool;
          static = true;
        }
      )
    ...
    liboil = composedArgsAndFun (selectVersion ../development/libraries/liboil "0.3.15") {
      inherit fetchurl stdenv pkgconfig;
      inherit (gtkLibs) glib;
    };
    ...
    */
    ...
    libxml2Python = lowPrio (
      appendToName "with-python" (
        import ../development/libraries/libxml2 {
          inherit
            fetchurl
            stdenv
            zlib
    ...
    libixp_for_wmii = lowPrio (
      import ../development/libraries/libixp_for_wmii {
        inherit fetchurl stdenv;
      }
    );
    ...
    # failed to build
    mediastreamer =
      composedArgsAndFun (selectVersion ../development/libraries/mediastreamer "2.2.0-cvs20080207")
        {
          inherit
            fetchurl
            stdenv
    ...
    mesaSupported = system == "i686-linux" || system == "x86_64-linux";

    mesa =
      assert mesaSupported;
      import ../development/libraries/mesa {
        inherit
          fetchurl
          stdenv
          pkgconfig
    ...
    ode =
      builderDefsPackage (import ../development/libraries/ode)
        {
        };
    ...
    unixODBCDrivers = recurseIntoAttrs (
      import ../development/libraries/unixODBCDrivers {
        inherit
          fetchurl
          stdenv
          unixODBC
    ...
    */
    ...
    perlCatalystAuthenticationStoreDBIxClass = buildPerlPackage rec {
      ...
      propagatedBuildInputs = [
        perlCatalystRuntime
        perlCatalystPluginAuthentication
        perlCatalystModelDBICSchema
      ];
    };
    ...
    perlCatalystControllerHTMLFormFu = buildPerlPackage rec {
      ...
      propagatedBuildInputs = [
        ...
        perlCatalystViewTT
        perlCatalystPluginConfigLoader
        perlConfigGeneral
        perlCatalystComponentInstancePerContext
        perlMoose
        perlRegexpAssemble
        perlTestWWWMechanize
        perlTestWWWMechanizeCatalyst
        perlHTMLFormFu
      ];
    };
    ...
    perlCatalystDevel = buildPerlPackage rec {
      ...
      propagatedBuildInputs = [
        perlCatalystRuntime
        perlCatalystActionRenderView
        perlCatalystPluginStaticSimple
        perlCatalystPluginConfigLoader
        perlPathClass
        perlTemplateToolkit
        perlClassAccessor
        perlConfigGeneral
        perlFileCopyRecursive
        perlParent
      ];
    };
    ...
    perlCatalystModelDBICSchema = buildPerlPackage {
      ...
      propagatedBuildInputs = [
        ...
        perlCatalystDevel
        perlDBIxClass
        perlUNIVERSALrequire
        perlClassDataAccessor
    ...
    perlCatalystRuntime = buildPerlPackage rec {
      ...
      propagatedBuildInputs = [
        ...
        perlNamespaceClean
        perlScopeUpper
        perlMooseXEmulateClassAccessorFast
        perlClassC3
        perlClassC3AdoptNEXT
      ];
    };
    ...
    perlDBIxClass = buildPerlPackage rec {
      ...
      propagatedBuildInputs = [
        ...
        perlPathClass
        perlClassInspector
        perlClassAccessorGrouped
        perlCarpClan
        perlTestWarn
        perlDataPage
        perlSQLAbstract
        perlSQLAbstractLimit
        perlClassC3
    ...
    perlHTMLFormFu = buildPerlPackage rec {
      ...
      propagatedBuildInputs = [
        ...
        perlDataVisitor
        perlDateTime
        perlDateTimeFormatBuilder
        perlDateTimeFormatStrptime
        perlDateTimeFormatNatural
        perlReadonly
        perlYAMLSyck
        perlRegexpCopy
        perlHTMLTokeParserSimple
        perlTestNoWarnings
        perlRegexpCommon
        perlCaptchaReCAPTCHA
        perlHTMLScrubber
        perlFileShareDir
        perlTemplateToolkit
        perlCryptCBC
    ...
    perlWWWMechanize = buildPerlPackage rec {
      ...
      propagatedBuildInputs = [
        perlLWP
        perlHTTPResponseEncoding
        perlHTTPServerSimple
      ];
      doCheck = false;
    };
    ...
    dictdDBs = recurseIntoAttrs (
      import ../servers/dict/dictd-db.nix {
        inherit builderDefs;
      }
    );
    ...
    squids = recurseIntoAttrs (
      import ../servers/squid/squids.nix {
        inherit
          fetchurl
          stdenv
          perl
    ...
    */
    ...
    kernel_2_6_22 = import ../os-specific/linux/kernel/linux-2.6.22.nix {
      ...
      extraConfig =
        lib.optional (getConfig [
          "kernel"
          "no_hz"
        ] false) "CONFIG_NO_HZ=y"
        ++ lib.optional (getConfig [
          "kernel"
          "timer_stats"
        ] false) "CONFIG_TIMER_STATS=y"
        ++ lib.optional (getConfig [
          "kernel"
          "usb_suspend"
        ] false) "CONFIG_USB_SUSPEND=y"
        ++ lib.optional (getConfig [
          "kernel"
          "no_irqbalance"
        ] false) "# CONFIG_IRQBALANCE is not set"
        ++ [
          (getConfig [
            "kernel"
            "addConfig"
    ...
    */
    ...
    kernelPackagesFor = kernel: rec {
      ...
      iwlwifi4965ucode = (
        if
          (builtins.compareVersions kernel.version "2.6.27" == 0)
      ...
      splashutils =
        if kernel.features ? fbSplash then
          splashutils_13
        else if kernel.features ? fbConDecor && system != "x86_64-linux" then
          splashutils_15
        else
          null;
    ...
    customKernel = composedArgsAndFun (
      lib.sumTwoArgs (import ../os-specific/linux/kernel/generic.nix) {
        inherit
          fetchurl
          stdenv
          perl
    ...
    */
    ...
    utillinuxStatic = lowPrio (
      appendToName "static" (
        import ../os-specific/linux/util-linux {
          inherit fetchurl;
    ...
    arkpandora_ttf =
      builderDefsPackage (import ../data/fonts/arkpandora)
        {
        };
    ...
    libertineBin =
      builderDefsPackage (selectVersion ../data/fonts/libertine "2.7.bin")
        {
        };

    lmodern =
      builderDefsPackage (selectVersion ../data/fonts/lmodern "1.010")
        {
        };
    ...
    carrier =
      builderDefsPackage (selectVersion ../applications/networking/instant-messengers/carrier "2.5.0")
        {
          inherit
            fetchurl
            stdenv
    ...
    compiz_050 =
      assert mesaSupported;
      import ../applications/window-managers/compiz/0.5.0.nix {
        inherit
          fetchurl
          stdenv
          pkgconfig
    ...
    compizBase =
      composedArgsAndFun
        (
          assert mesaSupported;
          selectVersion ../applications/window-managers/compiz "0.8.0"
        )
        {
    ...
    compizFusion =
      assert mesaSupported;
      import ../applications/window-managers/compiz-fusion {
        version = getConfig [
          "compizFusion"
          "version"
        ] "0.7.8";
    ...
    codeville =
      builderDefsPackage (selectVersion ../applications/version-management/codeville "0.8.0")
        {
          inherit makeWrapper;
          python = pythonFull;
        };
    ...
    #};
    eclipseNew = (
      selectVersion ../applications/editors/eclipse-new "3.3.1.1" {
    ...
    emacsUnicode = lowPrio (
      import ../applications/editors/emacs-unicode {
        inherit
          fetchcvs
          stdenv
          ncurses
    ...
    firefox2 = lowPrio (
      import ../applications/networking/browsers/firefox-2 {
        inherit
          fetchurl
          stdenv
          pkgconfig
    ...
    firefox3 = lowPrio (
      import ../applications/networking/browsers/firefox-3 {
        inherit
          fetchurl
          stdenv
          pkgconfig
    ...
    xulrunner3 = lowPrio (
      import ../applications/networking/browsers/firefox-3/xulrunner.nix {
        inherit
          fetchurl
          stdenv
          pkgconfig
    ...
    firefox3_1 = lowPrio (
      import ../applications/networking/browsers/firefox-3/3.1.nix {
        inherit
          fetchurl
          stdenv
          pkgconfig
    ...
    xulrunner3_1 = lowPrio (
      import ../applications/networking/browsers/firefox-3/xulrunner-3.1.nix {
        inherit
          fetchurl
          stdenv
          pkgconfig
    ...
    firefox3b1Bin = lowPrio (
      import ../applications/networking/browsers/firefox-3/binary.nix {
        inherit
          fetchurl
          stdenv
          pkgconfig
    ...
    gitAndTools = recurseIntoAttrs (
      import ../applications/version-management/git-and-tools {
        inherit pkgs;
      }
    );
    git = gitAndTools.git;
    ...
    gnash =
      assert mesaSupported;
      import ../applications/video/gnash {
        inherit
          fetchurl
          stdenv
          SDL
    ...
    icecat3Xul =
      (symlinkJoin "icecat-3-with-xulrunner" [
        icecat3
        icecatXulrunner3
      ])
      // {
    ...
    */
    ...
    mercurial = import ../applications/version-management/mercurial {
      inherit
        ...
        tk
        ;
      guiSupport = getConfig [
        "mercurial"
        "guiSupport"
      ] false; # for hgk (gitk gui for hg)
    };
    ...
    mozplugger =
      builderDefsPackage (import ../applications/networking/browsers/mozilla-plugins/mozplugger)
        {
          inherit firefox;
          inherit (xlibs) libX11 xproto;
        };
    ...
    nanoDiet = lowPrio (
      appendToName "diet" (
        import ../applications/editors/nano {
    ...
    pidginlatex =
      composedArgsAndFun
        (import ../applications/networking/instant-messengers/pidgin-plugins/pidgin-latex)
        {
          inherit
            fetchurl
            stdenv
    ...
    qemuImage = composedArgsAndFun (selectVersion ../applications/virtualization/qemu/linux-img "0.2") {
      inherit builderDefs fetchurl stdenv;
    };
    ...
    subversion14 = makeOverridable (import ../applications/version-management/subversion-1.4.x) {
      ...
      sslSupport = getConfig [
        "subversion"
        "sslSupport"
      ] true;
      pythonBindings = getConfig [
        "subversion"
        "pythonBindings"
      ] false;
      perlBindings = getConfig [
        "subversion"
        "perlBindings"
      ] false;
      javahlBindings = getConfig [
        "subversion"
        "javahlBindings"
      ] false;
      compressionSupport = getConfig [
        "subversion"
        "compressionSupport"
      ] true;
      httpd = apacheHttpd;
    ...
    subversionStatic = lowPrio (
      appendToName "static" (
        import ../applications/version-management/subversion-1.5.x {
          inherit
            fetchurl
            stdenv
            apr
    ...
    */
    ...
    uucp =
      builderDefsPackage (selectVersion ../tools/misc/uucp "1.07")
        {
        };
    ...
    vimDiet = lowPrio (
      appendToName "diet" (
        import ../applications/editors/vim-diet {
          inherit fetchurl;
          ncurses = ncursesDiet;
          stdenv = useDietLibC stdenv;
        }
    ...
    */
    ...
    wrapFirefox =
      browser: browserName: nameSuffix:
      import ../applications/networking/browsers/firefox-wrapper {
        inherit
          stdenv
          nameSuffix
          makeWrapper
    ...
    */
    ...
    qgis = (selectVersion ../applications/misc/qgis "0.11.0") {
      inherit
        ...
        glibc
        fontconfig
        freetype # use libc from stdenv ? - to lazy now - Marc
        ;
      inherit (xlibs)
        libSM
        libXcursor
    ...
    exultSnapshot = lowPrio (
      import ../games/exult/snapshot.nix {
        inherit
          fetchurl
          stdenv
          SDL
    ...
    */
    ...
    gnome = recurseIntoAttrs (
      import ../desktops/gnome {
        inherit
          fetchurl
          stdenv
          pkgconfig
    ...
    */
    ...
    ghostscriptX = lowPrio (
      appendToName "with-X" (
        import ../misc/ghostscript {
          inherit
            fetchurl
            stdenv
            libjpeg
    ...
    nixCustomFun =
      src: preConfigure: enableScripts: configureFlags:
    ...
    psi = builderDefsPackage (selectVersion ../applications/networking/instant-messengers/psi "0.12") {
      inherit
        builderDefs
        zlib
        aspell
